...
const browseEvent$ = S.snapshot(
  (interaction, { primaryContentIdM = M.empty(), contentTypeM = Option.none, pathname, href }) => {
    var _a, _b, _c, _d
    ;(_a = performance.measure) === null || _a === void 0
      ? void 0
      : _a.call(
          performance,
          CONTENT_LOAD_TIME,
    ...
    ;(_b = performance.measure) === null || _b === void 0
      ? void 0
      : _b.call(performance, SCREEN_PAINT_TIME, PerformanceMarkers.request_done)
    const contentLoadTime = fromMaybe(
      head(performance.getEntriesByName(CONTENT_LOAD_TIME).map((x) => Math.round(x.duration))),
      0
    )
    const screenPaintTime = fromMaybe(
      head(performance.getEntriesByName(SCREEN_PAINT_TIME).map((x) => Math.round(x.duration))),
      0
    )
  ...
  S.startWith(null, S.sample(interactionEvent$, maestroHistory.history.changes())),
  _browseEvent$
)
