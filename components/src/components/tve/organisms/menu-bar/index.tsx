...
import { PageTemplate, isPageTemplate } from "@discovery/common-tve/lib/constants"
import {
  useCallback,
  useState,
  useHistory,
...
export const MenuBar = ({
  ...
  const isSecondary =
    isPlayerPage || (isPageTemplate(templateId) && templateId === PageTemplate.Secondary)
    ...
    <>
      ...
      <nav className={classes}>
        ...
        <div className={cn(styles.navBarInner, styles.fill)}>
          <SiteLogo logoM={M.alt(logoM, mobileLogoM)} className={styles.desktopLogo} />
          <SiteLogo logoM={M.alt(mobileLogoM, logoM)} className={styles.mobileLogo} />
          ...
          <div className={cn(styles.flexed, styles.rightLinks)}>
            <EventDataProvider contentIndex={L.length(linksL) + 1}>
              <UserMenu authURL={authURLWithLocation} user={user} partnerM={partnerM} />
            </EventDataProvider>
          </div>
        </div>
      </nav>
