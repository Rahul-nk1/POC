...
import {
  GoToNext,
  GoToPrev,
} from "../../../../components/tve/atoms/carousel-nav";
...
const getProps = (data: CollectionResponseData) => {
  ...
  const heroMobileImageM = Fold.preview(
    Fold.compose(Optics._Col2Items, _MobileImage),
    data
  );
...
const Hero = ({
  ...
  const isPlaylist = useMemo(
    () =>
      componentId === PageComponentId.PlaylistPage ||
      templateId === HeroTemplate.Playlist,
  ...
  const heroItemLimitL = useMemo(
    () => (isCarousel ? heroData : L.take(1, heroData)),
    [isCarousel, heroData]
  );
  ...
  const activeVideoForShowAlternateIdM = useMemo(
    () => (!isCarousel ? showAlternateIdM : O.none),
    [isCarousel, showAlternateIdM]
  );
